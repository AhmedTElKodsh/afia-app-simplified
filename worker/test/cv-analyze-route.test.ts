...
import { cvAnalyzeRoute } from "../src/routes/cv-analyze.js";
...
describe("POST /api/cv-analyze integration", () => {
  ...
  diagnosticsApp.post("/api/cv-analyze", cvAnalyzeRoute);
  ...
  it("returns 400 if imageBase64 is missing", async () => {
    const res = await diagnosticsApp.request("/api/cv-analyze", {
      method: "POST",
      body: JSON.stringify({}),
  ...
  it("keeps the deployed Stage 1 worker CV diagnostics disabled by default", async () => {
