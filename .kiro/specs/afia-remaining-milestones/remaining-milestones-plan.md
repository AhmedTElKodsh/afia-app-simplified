...
- Research/party-mode conclusion, 2026-05-21: API-only is technically constrained now, but accuracy is still not good enough. Next best step is not more prompt-only work; it is using local/CV geometry to propose the liquid line, then asking the LLM to validate or explain evidence around that candidate. Treat cleaned natural images as the decision-grade eval source, modified/augmented images as robustness training/stress data, and require separate metrics for each.
...
- M007 should begin with a candidate-line proposal engine and leakage-safe manifests before any new model is allowed to claim improvement.
...
| M008 | Stage 2 hybrid runtime | Can the local model become the primary supported path with Gemini/OpenRouter/Grok fallback? | Common valid 1.5L captures complete through local primary with explainable fallback. |
